...
namespace Mogre
{
	...
	{
		//Nested Types
		public: ref class BoneBlendMask;
	
		#define STLDECL_MANAGEDTYPE float
		#define STLDECL_NATIVETYPE float
		public: INC_DECLARE_STLVECTOR( BoneBlendMask, STLDECL_MANAGEDTYPE, STLDECL_NATIVETYPE, public: , private: )
	...
	public:
		...
		void CreateBlendMask( size_t blendMaskSizeHint, float initialWeight );
		void CreateBlendMask( size_t blendMaskSizeHint );
	
		void DestroyBlendMask( );
	
		void _setBlendMaskData( const float* blendMaskData );
	
		void _setBlendMask( Mogre::AnimationState::Const_BoneBlendMask^ blendMask );
	
		Mogre::AnimationState::Const_BoneBlendMask^ GetBlendMask( );
