...
namespace Mogre
{
	...
	bool Archive::IsReadOnly::get()
	{
		return static_cast<const Ogre::Archive*>(_native)->isReadOnly( );
	}
	...
	void Archive::_Init_CLRObject( )
	{
		static_cast<Ogre::Archive*>(_native)->_Init_CLRObject( );
	}
	...
	Mogre::DataStreamPtr^ Archive::Open( String^ filename, bool readOnly )
	{
		DECLARE_NATIVE_STRING( o_filename, filename )
	
		return static_cast<const Ogre::Archive*>(_native)->open( o_filename, readOnly );
	}
	...
	Mogre::DataStreamPtr^ Archive::Create( String^ filename )
	{
		DECLARE_NATIVE_STRING( o_filename, filename )
	
		return static_cast<const Ogre::Archive*>(_native)->create( o_filename );
	}
	
	void Archive::Remove( String^ filename )
	{
		DECLARE_NATIVE_STRING( o_filename, filename )
	
		static_cast<const Ogre::Archive*>(_native)->remove( o_filename );
	}
	...
	time_t Archive::GetModifiedTime( String^ filename )
	{
		DECLARE_NATIVE_STRING( o_filename, filename )
	
		return static_cast<Ogre::Archive*>(_native)->getModifiedTime( o_filename );
	}
	...
	__declspec(dllexport) void _Init_CLRObject_Archive(CLRObject* pClrObj)
	{
		*pClrObj = gcnew Mogre::Archive(pClrObj);
	}
