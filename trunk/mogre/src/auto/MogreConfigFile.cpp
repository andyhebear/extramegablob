...
namespace Mogre
{
	...
	String^ ConfigFile::GetSetting( String^ key, String^ section, String^ defaultValue )
	{
		DECLARE_NATIVE_STRING( o_key, key )
		DECLARE_NATIVE_STRING( o_section, section )
		DECLARE_NATIVE_STRING( o_defaultValue, defaultValue )
	
		return TO_CLR_STRING( static_cast<const Ogre::ConfigFile*>(_native)->getSetting( o_key, o_section, o_defaultValue ) );
	}
