...
namespace Mogre
{
	...
	String^ DDSCodec::MagicNumberToFileExt( const char* magicNumberPtr, size_t maxbytes )
	{
		return TO_CLR_STRING( static_cast<const Ogre::DDSCodec*>(_native)->magicNumberToFileExt( magicNumberPtr, maxbytes ) );
	}
	...
	__declspec(dllexport) void _Init_CLRObject_DDSCodec(CLRObject* pClrObj)
	{
		*pClrObj = gcnew Mogre::DDSCodec(pClrObj);
	}
