...
namespace Mogre
{
	...
	void ParticleSystemManager::RemoveTemplatesByResourceGroup( String^ resourceGroup )
	{
		DECLARE_NATIVE_STRING( o_resourceGroup, resourceGroup )
	
		static_cast<Ogre::ParticleSystemManager*>(_native)->removeTemplatesByResourceGroup( o_resourceGroup );
	}
	...
	void ParticleSystemFactory::_Init_CLRObject( )
	{
		static_cast<Ogre::ParticleSystemFactory*>(_native)->_Init_CLRObject( );
	}
	...
	__declspec(dllexport) void _Init_CLRObject_ParticleSystemFactory(CLRObject* pClrObj)
	{
		*pClrObj = gcnew Mogre::ParticleSystemFactory(pClrObj);
	}
