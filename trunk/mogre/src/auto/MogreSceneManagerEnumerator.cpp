...
namespace Mogre
{
	...
	void DefaultSceneManagerFactory::_Init_CLRObject( )
	{
		static_cast<Ogre::DefaultSceneManagerFactory*>(_native)->_Init_CLRObject( );
	}
	...
	__declspec(dllexport) void _Init_CLRObject_DefaultSceneManagerFactory(CLRObject* pClrObj)
	{
		*pClrObj = gcnew Mogre::DefaultSceneManagerFactory(pClrObj);
	}
	...
	bool SceneManagerEnumerator::HasSceneManager( String^ instanceName )
	{
		DECLARE_NATIVE_STRING( o_instanceName, instanceName )
	
		return static_cast<const Ogre::SceneManagerEnumerator*>(_native)->hasSceneManager( o_instanceName );
	}
