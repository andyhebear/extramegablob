...
namespace Mogre
{
	...
	WindowEventListener::WindowEventListener() : Wrapper( (CLRObject*)0 )
	{
		...
		proxy->_overriden = Implementation::SubclassingManager::Instance->GetOverridenMethodsArrayPointer(thisType, WindowEventListener::typeid, 5);
		_native = proxy;
	...
	bool WindowEventListener::WindowClosing( Mogre::RenderWindow^ rw )
	{
		return static_cast<WindowEventListener_Proxy*>(_native)->WindowEventListener::windowClosing( rw );
	}
	...
	bool WindowEventListener_Proxy::windowClosing( Ogre::RenderWindow* rw )
	{
		if (_overriden[ 2 ])
		{
			bool mp_return = _managed->WindowClosing( rw );
			return mp_return;
		}
		else
